...
@pytest.mark.asyncio
async def test_basic_generate_goodbye():
...
async def test_custom_name():
    ...
    result = await generate_goodbye(template_args={"name": "Alice"})
...
async def test_custom_location():
    ...
    result = await generate_goodbye(template_args={"location": "Paris"})
...
@pytest.mark.asyncio
async def test_timezone_option():
    """Test timezone option"""
    result = await generate_goodbye(timezone="America/New_York")
    assert isinstance(result, str)
    assert len(result) > 0


@pytest.mark.asyncio
async def test_use_emojis_false():
...
@pytest.mark.asyncio
async def test_long_name_truncation():
    """Test that long names are truncated to 50 characters"""
    long_name = "A" * 100
    result = await generate_goodbye(template_args={"name": long_name})
